...
export type SDJ<S extends string, D = Record<string, unknown>> = { schema: S } & SelfDescribingJson<D>;

export type Event<S extends keyof typeof Events = keyof typeof Events> = {
  INTERVENTION_RECEIVED: SDJ<(typeof Events)[S], { handlers: string[] }>;
  INTERVENTION_HANDLE: SDJ<(typeof Events)[S], { handler: string }>;
  INTERVENTION_HANDLE_ERROR: SDJ<(typeof Events)[S], { handler: string; error?: string }>;
}[S];
...
export type MeasurementPayload<E extends MeasurementEvents> = Event<
  {
    [P in keyof typeof Events]: (typeof MEASUREMENT_EVENTS)[E] extends (typeof Events)[P] ? P : never;
  }[keyof typeof Events]
>;
